...
| G3 — Dynamic runtime alpha | Runtime metadata and client-surface interpreter pass lifecycle fixtures | Business catalog/browse/search/record transport is implemented against the observed route family; the authority-partition cache primitive exists but is not yet wired into the transports, and the client-surface half stays blocked on G1 |
